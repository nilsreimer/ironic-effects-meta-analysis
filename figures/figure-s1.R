...
# Figure S1 ---------------------------------------------------------------
  ...
  d_s1 <- read_rds("results/results_preregistered_analyses.rds") %>% 
    filter(x_var == "ic") %>% 
    group_by(y_var) %>% 
    median_qi(r_mean, mu) %>% 
    select(-starts_with("mu")) %>% 
    left_join(
      d_s1, ., by = c("y_var", ".width", ".point", ".interval")
    )
  ...
  d_s1 <- results %>% 
    ...
    left_join(d_s1, by = c("y_var", "y_name", "moderator"))
  
  # Visualize
  for (m in unique(d_s1$moderator)) {
    fig <- d_s1 %>% 
      filter(moderator == m) %>%
